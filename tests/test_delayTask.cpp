...
int main() {
    ...
    int nextDelay0 = 500;
    std::shared_ptr<toolkit::onceToken> token0 = std::make_shared<toolkit::onceToken>(nullptr, []() { TraceL << "task 0 被取消，可以立即触发释放lambad表达式捕获的变量!"; });
    ...
    int nextDelay3 = 50;
    auto tag3 = EventPollerPool::Instance().getPoller()->doDelayTask(nextDelay3, [&]() -> uint64_t { throw std::runtime_error("task 3(测试延时任务中抛异常,将导致不再继续该延时任务)"); });
