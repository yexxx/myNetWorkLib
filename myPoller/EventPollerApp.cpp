...
PipeEventPoller::PipeEventPoller(
    ...
    const std::function<void(const char *buf, int size)> cb)
    : _poller(poller) {
    if (!_poller) {
    ...
    _poller->addEvent(_pipe->readFd(), EventPoller::Event_Read,
                      [cb, pipe = _pipe](int) {
                          int nread = 1024;
...
Timer::Timer(float second, const EventPoller::Ptr &poller,
             const std::function<bool()> &cb)
    : _poller(poller) {
    if (!_poller) {
        _poller = EventPollerPool::Instance().getPoller();
    }
    _task = _poller->doDelayTask(
        static_cast<uint64_t>(second * 1000), [cb, second]() {
            try {
                if (cb()) {
